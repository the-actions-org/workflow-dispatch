...
export function getArgs() {
  ...
  const bypass = isBypass(bypassStr)
...
function isBypass(bypassStr: string | undefined) {
  if ( bypassStr === undefined || bypassStr.trim() === '') return true
  if ( bypassStr === 'true' ) return true
  if ( bypassStr === 'false' ) return false
  throw 'Invalid input.  Expected \'true\' or \'false\''
}
